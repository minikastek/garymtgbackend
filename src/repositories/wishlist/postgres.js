...
const { normalizeWishlist } = require('./model');
...
function mapWishlist(row, cards = []) {
...
class PostgresWishlistRepository {
  ...
  async listByUser(userId) {
    const result = await this.pool.query(
      "SELECT id, user_id, name, description, created_at, updated_at FROM collections WHERE user_id = $1 AND type = 'wishlist' ORDER BY created_at DESC", [userId]);
    const cards = await this.cardsFor(result.rows.map((row) => row.id));
    return result.rows.map((row) => mapWishlist(row, cards.get(row.id)));
  }

  async findById(id, client = this.pool) {
    const result = await client.query(
      "SELECT id, user_id, name, description, created_at, updated_at FROM collections WHERE id = $1 AND type = 'wishlist'", [id]);
    if (!result.rows[0]) return null;
    const cards = await this.cardsFor([id], client);
    return mapWishlist(result.rows[0], cards.get(id));
  }

  async create({ userId, name, description }) {
    const result = await this.pool.query(
      "INSERT INTO collections (id, user_id, type, name, description) VALUES ($1, $2, 'wishlist', $3, $4) RETURNING id, user_id, name, description, created_at, updated_at",
  ...
  async update(id, changes) {
    ...
    if (assignments.length) {
      values.push(id);
      await this.pool.query(`UPDATE collections SET ${assignments.join(', ')}, updated_at = now() WHERE id = $${values.length} AND type = 'wishlist'`, values);
    }
  ...
  async delete(id) {
    const result = await this.pool.query("DELETE FROM collections WHERE id = $1 AND type = 'wishlist'", [id]);
  ...
  async setCardQuantity(id, cardId, quantity) {
    const result = quantity === 0
      ? await this.pool.query('DELETE FROM collection_cards WHERE collection_id = $1 AND card_id = $2', [id, cardId])
      : await this.pool.query('UPDATE collection_cards SET quantity = $1, updated_at = now() WHERE collection_id = $2 AND card_id = $3', [quantity, id, cardId]);
    if (!result.rowCount) return undefined;
    await this.touch(id);
    return this.findById(id);
  }

  async touch(id) { await this.pool.query('UPDATE collections SET updated_at = now(), version = version + 1 WHERE id = $1', [id]); }
  removeCard(id, cardId) { return this.setCardQuantity(id, cardId, 0); }
}

module.exports = { PostgresWishlistRepository };
